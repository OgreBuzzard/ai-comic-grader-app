...
(function () {
  ...
  function buildCardDisplay(item) {
    ...
    // Show the version that PERFORMED this assessment (stored on the item),
    // not the current app version. "card-v1" was an old placeholder — skip it.
    const _storedVer = (rg && rg.version && rg.version !== "card-v1" && rg.version)
      || (card && card.version)
      || (card && card.robograde && card.robograde.version)
      || item.rgVersion;
    const _verLabel = "V" + (_storedVer || window.RG_GRADING_VERSION || "4.79");
