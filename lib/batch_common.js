...
// ⚠ SET TO false 2026-09-14 AFTER THE FIRST LIVE BATCH. With it true, all five
// Deeps were handed the SAME initialAssessment.grade, so they got an identical
// ±2 bracket AND an identical gradeTierContext() block in the prompt. Result:
// five Mains at 4.5/4.5/4.5/4.5/5.0 — healthy, credible variance — all collapsed
// to exactly PG 4.0 by the Deeps. The §4.3 risk fired exactly as written.
// Each Deep now calibrates off its OWN Main, which is the normal pipeline.
// There is NO cost trade here, contrary to what I first wrote. assess_deep.js now
// splits its system prompt at §§CACHE_SPLIT§§ the way assess.js has for months, so
// the cached prefix is the static ~70% that carries no book or grade in it. The
// per-pass grade lives in the uncached remainder, where it costs the same whether
// it is shared or not. Fixing the bracket bought nothing and cost all the variance.
export const BATCH_FIXED_BRACKET = false;
...
export function newBatchDoc({ uid, itemId, n, refs, anchorGrade, version }) {
  return {
    ...
    // passes is a MAP keyed by pass number, not an array. Five workers run
    // concurrently and each writes ONLY its own pass, so giving every pass its
    // own Firestore field path means their writes can never collide. As an
    // array it was a read-modify-write of one shared field by five writers —
    // a lost-update race that would silently drop finished passes.
    passes: Object.fromEntries(Array.from({ length: n }, (_, i) => [String(i + 1), {
...
// passes is stored as a map; callers that want ordered rows use this.
export function passList(batch) {
  const m = (batch && batch.passes) || {};
