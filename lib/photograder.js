...
export const PHOTOGRADER_RUBRIC_MAIN = `PHOTOGRADER — grade the QUALITY of the PHOTOGRAPHS (not the comic) in four categories, each A, B, or C. The question is whether each photo is a proper grading photo — the whole book, square to the camera, sharp, and readable — or whether its quality would reduce how accurately YOU can grade. A DELIBERATE, WELL-FRAMED photo is an A even if the light is warm/bright or a little background shows around the book — do not nitpick a good-faith grading shot. But you MUST grade down a careless snapshot: one that is blurry, shot at an angle, has the book small in the frame or surrounded by clutter (desk, keyboard, hands, lap, other objects), or has glare/shadow hiding detail. Do not hand out straight A's to a clearly poor photo. Use the WORST single photo for each category, and judge each category independently.
  FOCUS — grade whether the CAPTURE is in focus, NOT how sharply the ARTWORK was printed. Vintage newsprint, halftone-dot printing, and painted/airbrushed covers look inherently soft even in a perfectly focused photo — do NOT dock a well-focused shot for the era's print quality. A: the capture is in focus — edges, logo, small cover text, and staples are as crisp as the printing allows (a sharply-shot vintage book is an A even though its art is soft). B: the capture itself is soft — focus missed slightly, so detail the print DOES contain is smeared. C: visibly blurry from missed focus or camera motion; detail is lost.
  LIGHTING — A: the surface reads clearly (even, warm, bright, or a mild glossy/warm sheen are all fine — a glossy cover can throw a warm reflection and still be perfectly readable, and that is an A). B: the light is genuinely DIM, or a sheen/hotspot is strong enough that it actually OBSCURES part of the surface so defects under it can't be read. C: glare or shadow HIDES a meaningful area of the book. Judge lighting ONLY by whether detail is actually hidden — the mere presence of a warm sheen or glossy reflection is NOT a fault; do not drop to B unless that sheen genuinely blocks you from reading the surface. Spine, edge, and staple shots often use intentional raking/directional light to reveal surface defects — that is correct technique, NOT a lighting fault: grade these only on whether detail is HIDDEN (C), never dock them for uneven light alone.
