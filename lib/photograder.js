...
For each category return the letter. For every category graded B or C, add an object to the "flags" array: {"category": one of focus|lighting|cropping|angle (lowercase), "image": the photo name (e.g. "Front Cover", "Back Cover", "Interior", "Spine"), "note": a TERSE problem phrase, no prose}. Example flags: {"category":"lighting","image":"Back Cover","note":"glare hiding lower half"}, {"category":"cropping","image":"Front Cover","note":"too much table, crop in"}. A/graded categories get no flag.`;

// Model-facing rubric for DEEP/FULL passes (close-ups only → Focus/Lighting).
export const PHOTOGRADER_RUBRIC_CLOSEUP = `PHOTOGRADER — these are close-up photos, so grade only FOCUS and LIGHTING of THESE images, each A, B, or C (A ideal, B minor issue, C clear problem hiding detail). Use the worst image for each. Do NOT grade Cropping or Angle for close-ups. BUT if an image that should be a tight close-up instead shows more than about one quarter of the cover, flag it as "not close enough". Return the two letters, and for every category graded B or C add an object to the "flags" array: {"category": "focus" or "lighting", "image": the photo name, "note": a terse problem phrase, no prose}. Example: {"category":"focus","image":"Top-Left Corner","note":"blurry"}.`;
