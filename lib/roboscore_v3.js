...
(function (root) {
  ...
  function computeRoboScoreV3(input) {
    ...
    var uncapped = roundHalf(precise.front + precise.back + precise.spine + precise.interior);
    ...
    // Decompose the final 0.5 grade into 0.5 subscores that SUM to it exactly, so
    // the shown subscores always add up. Interior is fixed; front/back/spine
    // reconcile — front is shaved first (restored last), matching the cap order.
    function decomposeToGrade(p, g) {
      var interior = p.interior;
      var target = roundHalf(g - interior);
      var s2 = { front: clamp(roundHalf(p.front), 0, 5), back: clamp(roundHalf(p.back), 0, 2), spine: clamp(roundHalf(p.spine), 0, 2) };
      var MAX = { front: 5, back: 2, spine: 2 };
      // Round-robin (front -> back -> spine), matching the cap semantics: front is
      // shaved first each cycle (restored last). Add uses back -> spine -> front so
      // front is the last to be restored on the rare under-shoot.
      var shaveOrder = ["front", "back", "spine"], addOrder = ["back", "spine", "front"];
      function sum2() { return s2.front + s2.back + s2.spine; }
      var guard = 0, i = 0, j = 0;
      while (sum2() < target - 1e-9 && guard < 600) {
        var k = addOrder[j % addOrder.length];
        if (s2[k] + 0.5 <= MAX[k] + 1e-9) s2[k] = roundHalf(s2[k] + 0.5);
        j++; guard++;
        if (s2.front >= MAX.front && s2.back >= MAX.back && s2.spine >= MAX.spine) break;
      }
      while (sum2() > target + 1e-9 && guard < 600) {
        var kk = shaveOrder[i % shaveOrder.length];
        if (s2[kk] >= 0.5) s2[kk] = roundHalf(s2[kk] - 0.5);
        i++; guard++;
        if (s2.front <= 0 && s2.back <= 0 && s2.spine <= 0) break;
      }
      return { front: s2.front, back: s2.back, spine: s2.spine, interior: interior };
    }
    var sub = decomposeToGrade(precise, grade);
  ...
  function computeCardV3(input) {
    ...
    var uncapped = roundHalf(precise.surface + precise.corners + precise.edges + precise.centering);
    var cap = DEPTH_CAP_CARD[depth] != null ? DEPTH_CAP_CARD[depth] : 9.0;
    if (slabbed) cap = Math.min(cap, 9.5);
    var grade = Math.min(uncapped, cap);
    function decomposeCard(p, g) {
      var centering = p.centering;
      var target = roundHalf(g - centering);
      var s2 = { surface: clamp(roundHalf(p.surface), 0, 5), corners: clamp(roundHalf(p.corners), 0, 2), edges: clamp(roundHalf(p.edges), 0, 2) };
      var MAX = { surface: 5, corners: 2, edges: 2 };
      var shaveOrder = ["surface", "corners", "edges"], addOrder = ["corners", "edges", "surface"];
      function sum2() { return s2.surface + s2.corners + s2.edges; }
      var guard = 0, i = 0, j = 0;
      while (sum2() < target - 1e-9 && guard < 600) {
        var k = addOrder[j % addOrder.length];
        if (s2[k] + 0.5 <= MAX[k] + 1e-9) s2[k] = roundHalf(s2[k] + 0.5);
        j++; guard++;
        if (s2.surface >= MAX.surface && s2.corners >= MAX.corners && s2.edges >= MAX.edges) break;
      }
      while (sum2() > target + 1e-9 && guard < 600) {
        var kk = shaveOrder[i % shaveOrder.length];
        if (s2[kk] >= 0.5) s2[kk] = roundHalf(s2[kk] - 0.5);
        i++; guard++;
        if (s2.surface <= 0 && s2.corners <= 0 && s2.edges <= 0) break;
      }
      return { surface: s2.surface, corners: s2.corners, edges: s2.edges, centering: centering };
    }
    var sub = decomposeCard(precise, grade);
    var ceilingHit = uncapped > cap + 1e-9;
