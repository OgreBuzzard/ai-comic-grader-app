...
(function() {
  ...
  // Per-run first-photo delay (batch waits out a 3-coin drop). Set in runScanAnimation.
  let _firstPhotoDelayMs = null;
  ...
  function buildResultsPlaceholder(kind) {
    ...
    var _rgInit = _prev && _prev.rgV3 != null ? String(_prev.rgV3) : '0'; // S22: v3 grade, never the v1 0-100 score
    var _isCardScan = (typeof window !== 'undefined' && window._rgScanIsCard) || false;
    // Cards use a red PSA box (no decimals on whole grades); comics a blue GRADE box.
    var _gradeInit = _isCardScan
  ...
  async function runSequence(activeSlots, cancelToken) {
    const _fpd = (_firstPhotoDelayMs != null) ? _firstPhotoDelayMs : FIRST_PHOTO_DELAY;
    debugLog(`runSequence start, waiting firstPhotoDelay=${_fpd}ms`);
    await wait(_fpd, cancelToken);
